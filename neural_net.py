...
def avg_log_loss(An, Y):
    """
    Avg log-loss function
    reference: http://wiki.fast.ai/index.php/Log_Loss

    Args:
        An: last layer activations
        Y: true labels

    Returns:
        logloss
    """
    logprobs = Y * np.log(An) + (1 - Y) * np.log(1 - An)
...
def fit_params(X, Y, n_h, num_iterations=10000, learning_rate=1.2, debug=False):
    ...
    for idx in range(num_iterations):
        ...
        An, cache = forward_prop(X, params)

        cost = avg_log_loss(An, Y)
...
class NeuralNet:
    ...
    def predict(self, X):
        A2, _ = forward_prop(X, self.params)
        return A2 > .5
